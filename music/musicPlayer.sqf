...
if(battleIntensity > 10 ) then
{
    ...
    if(currentMusicState != "combat" || isMusicCurrentlyPlaying == 0) then
    {
        ...
        if(vehicle player != player) then
        {
            ...
            playMusic _selecter;
            duration =  getNumber (missionConfigFile >> "CfgMusic" >> _selecter >> "duration") + _combatTransitionDuration;
           // Проверка на то, что есть переменная продолжительности у музыки. Если нет, то убиваем проигрыватель  
			if(isNil "duration") exitwith {};
            if(debugging == 1) then
            {
    ...
    {  
        ...
        _eventForShotFiredNear = VehicleIamIn addEventHandler ["FiredNear", {_this execVM "Music\shotFiredNear.sqf"}];
        _eventForTakingDamage = VehicleIamIn addEventHandler ["Dammaged", {_this execVM "Music\damageTaken.sqf"}];
        _eventForExplosionsNearby = VehicleIamIn addEventHandler ["Explosion", {_this execVM "Music\explosionNear.sqf"}];
        _eventForMissileIncoming = VehicleIamIn addEventHandler ["IncomingMissile", {_this execVM "Music\missileIncoming.sqf"}];
...
/*          _this execVM "Music\shotFiredNear.sqf",
            _this execVM "Music\damageTaken.sqf",
            _this execVM "Music\explosionNear.sqf",
            _this execVM "Music\missileIncoming.sqf",
*/      }];
    ...
    if(battleIntensity <= 10) then
    {
			...
			if ( vehicle player isKindOf "car") then
			   ...
			   //in car based vehicles
                ...
                playMusic _selecter;
                duration =  getNumber (missionConfigFile >> "CfgMusic" >> _selecter >> "duration") + _safeTransitionDuration;
								// Проверка на то, что есть переменная продолжительности у музыки. Если нет, то убиваем проигрыватель  
			if(isNil "duration") exitwith {};
			...
			if ( vehicle player isKindOf "ship") then
			   ...
			   //in boat based vehicles
				...
				playMusic _selecter;
                _duration =  getNumber (missionConfigFile >> "CfgMusic" >> _selecter >> "duration") + _safeTransitionDuration;
								// Проверка на то, что есть переменная продолжительности у музыки. Если нет, то убиваем проигрыватель  
			if(isNil "duration") exitwith {};
    ...
    if(battleIntensity <= 10) then
    {
			...
			if ( vehicle player isKindOf "ship") then
			   ...
			   //in boat based vehicles
				_selecter = configName selectRandom _boatTracks;
				playMusic _selecter;
                _duration =  getNumber (missionConfigFile >> "CfgMusic" >> _selecter >> "duration") + 15;
								// Проверка на то, что есть переменная продолжительности у музыки. Если нет, то убиваем проигрыватель  
			if(isNil "duration") exitwith {};
